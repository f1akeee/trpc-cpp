...
int DoRpcCall(const std::shared_ptr<::trpc::test::helloworld::GreeterServiceProxy>& proxy, int n, int m) {
  ...
  if (!status.OK()) {
    std::cerr << "[" << n << "][" << m << "]: "
              << "get rpc error: " << status.ErrorMessage() << std::endl;
    return -1;
  }
  std::cout << "[" << n << "][" << m << "]: "
            << "get rsp msg: " << rsp.msg() << std::endl;
  return 0;
}
...
int Run() {
  for (int i{0}; i < 4; ++i) {
    ...
    std::chrono::milliseconds timespan(1000);  // 1000毫秒 = 1.0秒
